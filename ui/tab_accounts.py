...
class SinkingFundDialog(QDialog):
    ...
    def _build(self):
        ...
        annual_row.setSpacing(6)
        annual_row.addWidget(self.annual)
        unit_lbl = QLabel("円/年")
        unit_lbl.setStyleSheet(f"color:{COLORS['text_sub']};")
        annual_row.addWidget(unit_lbl)
        annual_widget = QWidget()
        annual_widget.setLayout(annual_row)
        layout.addRow("年間予算 *", annual_widget)
