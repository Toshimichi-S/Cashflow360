...
class BudgetTab(QWidget):
    ...
    def _build_ui(self):
        ...
        for text, width in [("種別", 55), ("項目", 160), ("", -1),
                             ("予算", 80), ("実績", 80), ("差異", 90)]:
            lb = QLabel(text)
            lb.setStyleSheet(f"color:{COLORS['text_dim']};font-size:11px;background:transparent;")
            if width > 0:
                lb.setFixedWidth(width)
                lb.setAlignment(Qt.AlignRight | Qt.AlignVCenter if width < 100 else Qt.AlignLeft | Qt.AlignVCenter)
            else:
                lb.setSizePolicy(lb.sizePolicy().horizontalPolicy(), lb.sizePolicy().verticalPolicy())
            hdr_row.addWidget(lb, 0 if width > 0 else 1)
        self.vl.addLayout(hdr_row)

        self.card_budget = make_card()
        self.budget_layout = QVBoxLayout(self.card_budget)
        self.budget_layout.setContentsMargins(0, 4, 0, 4)
        ...
        self.extra_layout.setContentsMargins(14, 8, 14, 8)
    ...
    def _prev_month(self):
        ...
        if dt.month == 1:
            self.year_month = f"{dt.year-1}-12"
        else:
            self.year_month = f"{dt.year}-{dt.month-1:02d}"
    ...
    def _next_month(self):
        ...
        dt = datetime.strptime(self.year_month, "%Y-%m")
        if dt.month == 12:
            self.year_month = f"{dt.year+1}-01"
        else:
            self.year_month = f"{dt.year}-{dt.month+1:02d}"
        self.refresh(self.year_month)
    ...
    def _refresh_income(self):
        while self.income_layout.count():
            item = self.income_layout.takeAt(0)
            if item.widget():
    ...
    def _refresh_budget(self):
        while self.budget_layout.count():
            item = self.budget_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
    ...
    def _refresh_extra(self):
        ...
        for e in extras:
            ...
            w = QWidget()
            w.setLayout(row)
            w.setStyleSheet(f"border-bottom:1px solid {COLORS['border']};background:transparent;")
...
class _IncomeRow(QWidget):
    ...
    def __init__(self, data: dict, category: str, parent=None):
        ...
        lb_amt.setStyleSheet(f"color:{COLORS['text']};background:transparent;")
        hl.addWidget(lb_amt)
...
class _BudgetItemRow(QWidget):
    ...
    def __init__(self, data: dict, parent=None):
        ...
        col = (COLORS["red"] if pct > 100 else
               ...
               COLORS["teal"] if itype == "sinking" else COLORS["blue"])
        bar.setStyleSheet(
        ...
        for text, color in [
            ...
            lb2.setStyleSheet(f"color:{color};background:transparent;border:none;")
            hl.addWidget(lb2)
        ...
        if itype not in ("fixed_auto", "sinking"):
...
class AddIncomeDialog(QDialog):
    ...
    def _build(self):
        ...
        self.amount.setSuffix(" 円")
        layout.addRow("予定額 *", self.amount)
...
class AddVariableBudgetDialog(QDialog):
    ...
    def _build(self):
        ...
        self.budget.setSuffix(" 円")
        layout.addRow("目標額 *", self.budget)
